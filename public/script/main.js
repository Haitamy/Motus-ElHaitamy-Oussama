...
let guesses = [];
let currentGuess = [];
let random = parseInt(Math.random() * mesMots.length);
console.log(random);
let randomWord = mesMots[random];
...
function initialize() {
  ...
  indice.innerHTML += SecretWord.charAt(0);
  ...
  Object.keys(keys).forEach((key) => {
    if (key.includes("break")) {
      ...
      keyboard.innerHTML +=
        `<button id="${key}" class="key" onclick="keyClick('${key}')">` +
        key +
        "</button>";
    }
  });
  document.addEventListener("keydown", function (event) {
    switch (event.code) {
      ...
      default:
        if (
          currentGuess.length < SecretWord.length &&
          guesses.length < NumberOfGuesses
        ) {
          currentGuess.push({ key: event.key, result: "" });
          updateCurrentGuess();
        }
    }
  });
